...
if __name__ == '__main__':
    ...
    for comparing_file in comparing_files:
        ...
        for channel in channels:
            ...
            # target_hsv_mask = cv2.inRange(target_hsv, hsvLower, hsvUpper)
            # target_fusion_hsv = cv2.bitwise_and(target_img, target_img, mask=target_hsv_mask)
            target_hist = cv2.calcHist([target_hsv], [0,1], None, [180,255], [0, 180, 1, 256])
            ...
            # comparing_hsv_mask = cv2.inRange(comparing_hsv, hsvLower, hsvUpper)
            # comparing_fusion_hsv = cv2.bitwise_and(comparing_img, comparing_img, mask=comparing_hsv_mask)
            comparing_hist = cv2.calcHist([comparing_hsv], [0,1], None, [180,255], [0, 180, 1, 256])
